...
PNODE MatchingNode (PNODE pNode, PSZ pszWord)
   ...
   if (bUSEDIVERGENCE)
      {
      if (Rnd(100) > (INT)uDIVERGENCE)
         return pNode;
      }
   else // else use the DEPTH method
      {
      /*--- definately ok to use this node ---*/
      if (DEPTH > pNode->uCount)
         return pNode;
      }
...
void Usage ()
   {
   printf ("   SCHIZO       Text file scrambler    v2.0                %s  %s\n\n", __DATE__, __TIME__);
   printf ("   USAGE: SCHIZO [options]  textfiles...\n\n");
   printf ("   WHERE: textfiles  .... Text files to read and contemplate.\n\n");
   printf ("           [options] are 0 or more of:\n");
   printf ("               Out=file ... Specify output filename (def is a.o)\n");
   printf ("               Seed=# ..... Specify random number seed.\n");
   printf ("               Depth=# .... Specify max # of diverging paths per word.\n");
   printf ("               Divergence=# Specify divergence % 0-100 [100 = orig file].\n\n");
   printf ("   EXAMPLE:  SCHIZO readme.txt\n\n");
   printf ("   In this example, the file readme.txt is read in and rearranged output\n");
   printf ("   to the file readme.out.\n");
   exit (0);
   }
...
int _cdecl main (int argc, char *argv[])
   ...
   if (ArgBuildBlk ("? *^Debug *^Out% *^Seed% *^Depth% *^Divergence%"))
      Error ("%s", ArgGetErr ());
   ...
   srand (uSeed);
   DEPTH        = (ArgIs ("Depth")    ? atoi (ArgGet ("Depth", 0)   ) : 500);
   MAXOUTLINES  = (ArgIs ("MaxLines") ? atoi (ArgGet ("MaxLines", 0)) : 5000);
   LINELEN      = (ArgIs ("LineLen")  ? atoi (ArgGet ("LineLen", 0) ) : 78);

   if (bUSEDIVERGENCE = ArgIs ("Divergence"))
      uDIVERGENCE = atoi (ArgGet ("Divergence", 0));

   pnfo = malloc (sizeof (NFO));
   InitNFO (pnfo);

   for (i=0; ArgIs (NULL) > i; i++)
      ReadFile (pnfo, ArgGet (NULL, i));
   ...
   DirMakeFileName (szOutFile, ArgGet ("Out", 0), ArgGet (NULL, 0), ".out");
